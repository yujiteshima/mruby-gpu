...
| [docs/GPU_VS_CPU_ANALYSIS.md](docs/GPU_VS_CPU_ANALYSIS.md) | GPU vs CPU 推論の比較調査・なぜ V3D で遅いのか |
| [docs/MINIBATCH_PLAN.md](docs/MINIBATCH_PLAN.md) | MNIST ミニバッチ化計画・GPU 高速化ロードマップ |
| [docs/BATCH_INFERENCE_PLAN.md](docs/BATCH_INFERENCE_PLAN.md) | 録画→バッチ推論パイプライン計画 |
| [docs/LT_SCRIPT.md](docs/LT_SCRIPT.md) | LT 発表スクリプト（台本・デモ手順） |
...
| `SkinDetector` | 肌色検出による人数カウント（学習モデル不要・CPU 軽量） |
...
mruby-gpu は 3 種類の検出器を提供します。**外部ライブラリに乗るもの**、**完全自作のルールベース**、**mruby 上で学習から推論まで組むもの** がそれぞれ含まれています。

| # | 検出器 | 推論エンジン | モデル出自 | 外部依存 |
|---|--------|-------------|-----------|---------|
| 1 | `FaceDetector` | **NCNN** ([Tencent](https://github.com/Tencent/ncnn)) | [UltraFace-slim](https://github.com/Linzaer/Ultra-Light-Fast-Generic-Face-Detector-1MB) (Linzaer) | libncnn + Vulkan / NEON |
| 2 | `SkinDetector` | 自作ロジックベース | — | なし（libc のみ） |
| 3 | MNIST MLP | 自作 mruby + Compute Shader | 自分で学習 | なし（自前 shader のみ） |
...
- 実装: [src/mrb_face.cpp](src/mrb_face.cpp)
- モデル: `models/ultraface-slim.{param,bin}`
- 実測: 165ms / 6 FPS（GPU V3D）vs 12ms / 82 FPS（CPU NEON） — 詳細は [docs/GPU_VS_CPU_ANALYSIS.md](docs/GPU_VS_CPU_ANALYSIS.md)

### ② `SkinDetector` — 完全自作・学習不要

RGB → YCbCr 変換 → 肌色色域の閾値判定 → グリッド化 → flood-fill で連結領域抽出。学習モデルゼロ、libc のみで動く超軽量実装。

- 実装: [src/mrb_skin.c](src/mrb_skin.c)
- 精度は照明条件に左右されるが、NCNN 不要で CPU 超高速
- 用途: [demo/counter_demo.rb](demo/counter_demo.rb)（`method: :skin`）
...
- 学習: [examples/train.rb](examples/train.rb) / [examples/train_minibatch.rb](examples/train_minibatch.rb)
- 推論: [examples/mnist_inference.rb](examples/mnist_inference.rb)（全 10000 枚 batched）
- シェーダー: [shader/matmul.comp](shader/matmul.comp) / [add.comp](shader/add.comp) / [relu.comp](shader/relu.comp) など
- 精度: ~97%（MNIST test）
